...
### 首页与遥控器焦点
- **默认焦点**：进入首页后刷新按钮自动获得焦点
- **刷新按钮**：无媒体资源时锁定左键（避免误下移）；下键在无内容时进入底部导航「首页」；有内容时正常进入媒体列表
- **底部导航**：首页首项与菜单键进入导航栏的焦点衔接优化

### 网页推送与提示流程
- **WebDAV 扫码添加**：自检通过后关闭二维码，弹出独立提示（下一步：首页资源库选择服务器并开始刮削）；提示与列表层级修正为 `Dialog`，避免被遮挡
- **TMDB API 扫码**：推送成功并提示后，自动关闭二维码窗口
- **直播源 / 节目表 / UA**：历史列表内点「添加」不关闭列表，二维码叠在列表之上；推送成功后先关二维码，**立即**弹出「添加成功」说明（按入口区分文案）；使用 `Dialog` 保证叠在列表之上；关闭提示后可在同一列表中选项，再确认关闭列表

### 其他
- 直播网页配置成功提示文案按「直播源 / 节目表 / UA」入口区分；列表在二维码或成功提示显示时屏蔽误触返回关闭
- 版本号与构建配置更新至 v1.0.12 / 10012

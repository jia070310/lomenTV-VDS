...
**构建号**：10017。安装包请从 [Releases](https://github.com/jia070310/lomenTV-VDS/releases) 下载；仓库内不再附带 APK 二进制。

### v1.0.17 更新简介

- **版本**：`versionName` **1.0.17**，`versionCode` **10017**。
- **资源库页面**：修复「当前有 0 个资源库」时，焦点在左上角返回键上按**右键**试图进入右侧添加区域会**闪退**的问题（空态下未挂载的 `FocusRequester` 上调用 `requestFocus()`）；空态「添加资源库」按钮正确挂载焦点，并与返回键建立左右导航。
